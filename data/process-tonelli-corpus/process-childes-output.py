...
with open('./data/process-tonelli-corpus/art-N-adj.txt', encoding="utf-8") as f:
    raw_DNA = f.read()

with open('./data/process-tonelli-corpus/art-N.txt', encoding="utf-8") as g:
    raw_DN = g.read()
...
print(len([x for x in Include_list if x.dptype == 'DN'])) #3739
...
# INPUT DATA STATS
Include_list_DN = [observation for observation in Include_list if observation.dptype == 'DN']
